...
pub async fn follow_user(pool: &PgPool, follower_id: Uuid, followee_id: Uuid) -> Result<()> {
    ...
    let result = sqlx::query_file!(
        "src/queries/user_follows/insert_follow.sql",
        follower_id,
        followee_id
    )
    .execute(&mut *tx)
    ...
    sqlx::query_file!(
        "src/queries/user_stats/increment_following.sql",
        follower_id
    )
    .execute(&mut *tx)
    .await?;
    ...
    sqlx::query_file!("src/queries/user_stats/increment_follower.sql", followee_id)
        .execute(&mut *tx)
        .await?;
...
pub async fn unfollow_user(pool: &PgPool, follower_id: Uuid, followee_id: Uuid) -> Result<()> {
    ...
    let result = sqlx::query_file!(
        "src/queries/user_follows/delete_follow.sql",
        follower_id,
        followee_id
    )
    .execute(&mut *tx)
    ...
    sqlx::query_file!(
        "src/queries/user_stats/decrement_following.sql",
        follower_id
    )
    .execute(&mut *tx)
    .await?;
    ...
    sqlx::query_file!("src/queries/user_stats/decrement_follower.sql", followee_id)
        .execute(&mut *tx)
        .await?;
...
pub async fn get_followers(
    ...
    let users = sqlx::query_file_as!(
        ...
        "src/queries/user_follows/get_followers.sql",
        login_user_id,
        target_user_id
    )
    .fetch_all(pool)
...
pub async fn get_followees(
    ...
    let users = sqlx::query_file_as!(
        crate::dtos::user_dto::UserProfileResponse,
        "src/queries/user_follows/get_followees.sql",
        login_user_id,
        target_user_id
    )
    .fetch_all(pool)
