...
function QuizStartContent() {
      ...
      const fetchPreselected = async () => {
        try {
          ...
          setSelectedCollections([
            { id: c.id, name: c.name, questionCount: c.questionCount || 0 },
          ]);
  ...
  // Quiz options state
  const [filterNode, setFilterNode] = useState<FilterNode | undefined>(
    undefined,
  );
  ...
  const totalQuestions = selectedCollections.reduce(
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      ...
      const maybeLoadModel =
        preferredMode === "fuzzy" && !scorer.isReady
          ? scorer.load()
          : Promise.resolve();
      ...
      if (isOnline) {
        // クイズ開始時に関連コレクションのメタ情報を補強キャッシュする
        void Promise.allSettled(
          selectedCollections.map(async (selected) => {
            const c = await getCollection(selected.id);
            await syncCollectionMetadata(c);
            ...
            if (c.userId && c.authorName) {
              ...
              await saveOfflineProfile(fallbackProfile);
            }
          }),
        );
      }
      ...
      {preferredMode === "fuzzy" && scorer.isLoading && (
        <View className="fixed bottom-28 left-0 right-0 z-40 flex justify-center px-4">
          <View className="flex items-center gap-3 bg-surface-base border border-brand-primary/30 rounded-xl px-5 py-3 shadow-lg max-w-sm w-full">
            ...
            <Text variant="xs" color="secondary" className="font-mono shrink-0">
              {scorer.progress}%
            </Text>
          </View>
        </View>
