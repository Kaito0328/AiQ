...
export default function CacheManagementPage() {
    ...
    const clearTable = async (tableName: keyof typeof db & ('collections' | 'questions' | 'profiles' | 'pendingActions')) => {
    ...
    const clearAll = async () => {
        if (!window.confirm('すべてのキャッシュを削除しますか？未送信のアクションも削除されます。')) return;
        ...
        try {
            ...
            showToast({ message: 'すべてのキャッシュを削除しました', variant: 'success' });
            fetchStats();
        } catch (err) {
            showToast({ message: '削除に失敗しました', variant: 'danger' });
        }
                ...
                <Stack gap="lg" className="max-w-2xl mx-auto">
                        ...
                        <Stack gap="xl">
                            <Flex justify="between" align="center">
                                <Flex gap="sm" align="center">
                                    ...
                                    <Text variant="h3" weight="bold">ストレージ統計</Text>
                                </Flex>
                    ...
                    <View className="bg-brand-primary/10 p-4 rounded-lg border border-brand-primary/20">
                        <Text variant="xs" color="primary" weight="bold">
                            Tips: 開発中のテストでデータが不整合になった場合や、ストレージ容量を節約したい場合に実行してください。
                        </Text>
                    </View>
                </Stack>
            </Container>
...
function StatItem({ label, count, onClear, icon, critical = false }: { 
    label: string, 
    count: number, 
    onClear: () => void,
    ...
    return (
        <Flex justify="between" align="center" className="p-3 bg-surface-muted rounded-lg border border-surface-muted">
            <Flex gap="sm" align="center">
                ...
                <Stack gap="none">
                    ...
                    <Text variant="xs" color="secondary">{count} 件保存済み</Text>
                </Stack>
            </Flex>
