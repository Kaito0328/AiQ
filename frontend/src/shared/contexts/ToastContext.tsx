...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const showToast = useCallback((props: Omit<ToastProps, "onClose">) => {
    ...
    setToasts((prev) => {
      const isDuplicate = prev.some(
        (t) => t.message === props.message && t.variant === props.variant,
      );
  ...
  return (
    <ToastContext.Provider value={{ showToast }}>
      ...
      <View className="fixed bottom-4 right-4 z-tooltip pointer-events-none">
        <Stack gap="sm">
          {toasts.map((toast) => (
            <View
              key={toast.id}
              className="pointer-events-auto animate-in slide-in-from-right-5 fade-in"
            >
