...
export class AiQDatabase extends Dexie {
    ...
    async deleteCollectionContent(collectionId: string) {
        await this.transaction('rw', [this.collections, this.questions], async () => {
            await this.questions.where('collectionId').equals(collectionId).delete();
     ...
     */
    async saveCollection(collection: Collection, questions: Question[], isExplicit: boolean = false) {
