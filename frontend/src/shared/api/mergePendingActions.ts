 ...
 */
export async function mergePendingCollections(
  ...
  for (const action of pending) {
    ...
    if (type === "DELETE_COLLECTION") {
      ...
      if (index !== -1) {
        result[index] = {
          ...result[index],
          isFavorited: type === "FAVORITE_COLLECTION",
        };
      }
    }
  }
 ...
 */
export async function mergePendingProfile(
  ...
  const pending = await db.pendingActions
    .where("type")
    .anyOf(["FOLLOW_USER", "UNFOLLOW_USER"])
    .toArray();
...
export async function mergePendingSets(
  ...
  const pending = await db.pendingActions
    .where("type")
    .anyOf(["CREATE_SET", "UPDATE_SET", "DELETE_SET"])
    .toArray();
