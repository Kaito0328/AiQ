...
class SyncManager {
  ...
  async sync() {
    ...
    try {
      for (const action of actions) {
        try {
          ...
          logger.info(
            `Successfully synced action: ${action.type}`,
            action.payload,
          );
        } catch (err: any) {
    ...
    const remainingPending = await db.pendingActions
      .where("status")
      .equals("pending")
      .count();
  ...
  async addAction(type: PendingActionType, payload: any) {
    ...
    if (type === "FOLLOW_USER" || type === "UNFOLLOW_USER") {
      ...
      const oppositeType =
        type === "FOLLOW_USER" ? "UNFOLLOW_USER" : "FOLLOW_USER";
      const oppositeIndex = existing.findIndex(
        (a) =>
          a.type === oppositeType &&
          (a.payload.userId === userId || a.payload === userId),
      );
      if (oppositeIndex !== -1) {
    ...
    if (type === "FAVORITE_COLLECTION" || type === "UNFAVORITE_COLLECTION") {
      ...
      const oppositeIndex = existing.findIndex(
        (a) =>
          a.type === oppositeType &&
          (a.payload.collectionId === collectionId ||
            a.payload === collectionId),
      );
      if (oppositeIndex !== -1) {
        await db.pendingActions.delete(existing[oppositeIndex].id!);
        return;
    ...
    if (type === "DELETE_COLLECTION") {
      ...
      const createIndex = existing.findIndex(
        (a) =>
          ...
          (a.payload.id === collectionId || a.payload === collectionId),
      );
      if (createIndex !== -1) {
    ...
    if (type === "DELETE_SET") {
      ...
      const createIndex = existing.findIndex(
        (a) =>
          a.type === "CREATE_SET" &&
          (a.payload.tempSet?.id === setId || a.payload.id === setId),
      );
      if (createIndex !== -1) {
        await db.pendingActions.delete(existing[createIndex].id!);
        return;
    ...
    if (type === "UPDATE_COLLECTION" || type === "UPDATE_QUESTION") {
      ...
      const duplicateIndex = existing.findIndex(
        (a) => a.type === type && a.payload[idKey] === entityId,
      );
      if (duplicateIndex !== -1) {
    ...
    if (type === "UPDATE_SET") {
      ...
      const duplicateIndex = existing.findIndex(
        (a) => a.type === "UPDATE_SET" && a.payload.id === setId,
      );
      if (duplicateIndex !== -1) {
        await db.pendingActions.delete(existing[duplicateIndex].id!);
      }
    ...
    if (type === "SUBMIT_QUIZ_RESULT") {
      ...
      const duplicateIndex = existing.findIndex(
        (a) => a.type === "SUBMIT_QUIZ_RESULT" && a.payload.quizId === quizId,
      );
      if (duplicateIndex !== -1) {
        const existingAction = existing[duplicateIndex];
        const answers = existingAction.payload.answers || [
          existingAction.payload.data,
        ];
        answers.push(data);
