...
const PageContainer: React.FC<PageContainerProps> = ({ children, backgroundClassName, className }) => {
  ...
  return (
    <div className={`min-h-screen ${backgroundClassName ?? 'bg-gray-100'} py-8 px-4 md:px-8 ${className}`}>
      <button
        onClick={() => navigate(-1)}
        className="fixed top-20 left-5 p-2 rounded-full bg-white/80 hover:bg-white shadow-md text-blue-600 hover:text-blue-800 transition"
        title="戻る"
      >
        <FaArrowLeft size={18} />
      </button>
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="fixed bottom-6 right-3 flex flex-col gap-3 z-40 items-end">
          {showScrollTop && (
            <button
