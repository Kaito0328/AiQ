...
export const useCollectionAPIActions = (
  ...
  const handleBatchDelete = useCallback(async (ids: number[], collectionSetId: number): Promise<BatchDeleteResponse<Collection>> => {
    const ok = window.confirm(`${ids.length}件を削除しますか？`);
    if (!ok) return { successItems: [], failedItems: [] };

    try {
      const res: BatchDeleteResponse<Collection> = await deleteCollections(ids, collectionSetId);
