import { deleteCollection, deleteCollections} from '../../../../../api/CollectionAPI';
...
import { getQuestionsByCollectionId, upsertQuestions } from '../../../../../api/QuestionAPI';
...
export const useAPIActions = (
  ...
  const handleBatchDelete = useCallback(async (questionIds: number[]): Promise<BatchDeleteResponse<Collection>> => {
    const ok = window.confirm(`${questionIds.length}件を削除しますか？`);
    if (!ok) return { successItems: [], failedItems: [] };

    try {
      const res: BatchDeleteResponse<Collection> = await deleteCollections(questionIds);
