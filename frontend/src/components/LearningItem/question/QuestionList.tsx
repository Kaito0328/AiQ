  ...
  const renderPendingItem = useCallback(
    (question: QuestionInput, index: number) => (
       <NewItemCard
        ...
        onDelete={() => deleteLocalCreate(index)}
      />
    ),
    [isOwner, handleLocalCreate, apiState.createErrors, deleteLocalCreate]
  );
  ...
  return (
    <div>
      ...
      <ItemList<Question, QuestionInput>
        ...
        onBatchDelete={() => handleBatchDelete(localState.selectedIds, collectionId)}
      />
      <div className="fixed bottom-9 right-10 z-30">
        <QuestionModal
