...
import {
  loadPendingEditRequestsCache,
  savePendingEditRequestsCache,
} from "@/src/shared/api/editRequestCache";
...
export function EditRequestReviewList({
  collectionId,
  onActionSuccess,
  isGlobal = false,
}: EditRequestReviewListProps) {
        ...
        editingRequestId === request.id
          ? {
              ...
              correctAnswers: editData.correctAnswers
                ...
                .map((a: string) => a.trim())
                .filter((a: string) => a !== ""),
              answerRubis: editData.answerRubis
                .split(";")
                .map((a: string) => a.trim())
                .filter((a: string) => a !== ""),
              distractors: editData.distractors
                ...
                .filter((a: string) => a !== ""),
              descriptionText:
                editData.descriptionText.trim() !== ""
                  ? editData.descriptionText
                  : undefined,
            }
          : {
              questionText: request.questionText,
                ...
                <View className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Stack gap="xs">
                    ...
                    <View
                      padding="md"
                      rounded="lg"
                      className="bg-surface-muted/30 border border-surface-muted opacity-60"
                    >
                      <Stack gap="sm">
                        <Text variant="detail" weight="medium">
                          {request.originalQuestionText}
                        </Text>
                        <Text variant="xs" color="secondary">
                          答:{" "}
                          {request.originalCorrectAnswers?.join(" / ") ||
                            "未設定"}
                        </Text>
                  ...
                  <Stack gap="xs">
                    <Flex justify="between" align="center">
                      ...
                      <Button
                        ...
                        size="sm"
                        color={
                          editingRequestId === request.id ? "danger" : "primary"
                        }
                        className="h-7 px-2"
                        onClick={() =>
                          editingRequestId === request.id
                            ? setEditingRequestId(null)
                            : handleStartEdit(request)
                        }
                      >
                        {editingRequestId === request.id ? (
                    ...
                    <View
                      ...
                      className={cn(
                        ...
                        editingRequestId === request.id
                          ? "bg-surface-base border-brand-primary ring-1 ring-brand-primary/20"
                          : "bg-brand-primary/5 border-brand-primary/20",
                      )}
                    >
                      <ArrowRight
                        size={20}
                        className="absolute -left-6 top-1/2 -translate-y-1/2 text-brand-primary hidden md:block"
                      />
                      {editingRequestId === request.id ? (
                        <Stack gap="md">
                          <Stack gap="xs">
                        ...
                        <Stack gap="sm">
                          <Text
                            ...
                            className={cn(
                              request.originalQuestionText !==
                                request.questionText && "text-brand-primary",
                            )}
                          >
                          ...
                          <Text
                            ...
                            className={cn(
                              request.originalCorrectAnswers?.join(";") !==
                                request.correctAnswers.join(";") &&
                                "text-brand-primary font-bold",
                            )}
                          >
                            答: {request.correctAnswers.join(" / ")}
                          </Text>
                          {(request.answerRubis?.length > 0 ||
                            request.originalAnswerRubis?.length > 0) && (
                            <Text
                              variant="xs"
                              className={cn(
                                request.originalAnswerRubis?.join(";") !==
                                  request.answerRubis?.join(";") &&
                                  "text-brand-primary font-bold",
                              )}
                            >
                              読み:{" "}
                              {request.answerRubis?.join(" / ") || "(なし)"}
                            </Text>
                          )}
                          {(request.distractors?.length > 0 ||
                            request.originalDistractors?.length > 0) && (
                            <Text
                              variant="xs"
                              className={cn(
                                request.originalDistractors?.join(";") !==
                                  request.distractors?.join(";") &&
                                  "text-brand-primary font-bold",
                              )}
                            >
                              選択肢:{" "}
                              {request.distractors?.join(" / ") || "(なし)"}
                            </Text>
                          )}
