  ...
  const toggleSort = (key: SortKey) => {
    ...
    if (existing) {
      ...
      if (existing.direction === SortDirection.ASC) {
        ...
        onSortChange(
          sorts.map((s) =>
            s.key === key ? { ...s, direction: SortDirection.ASC } : s,
          ),
        );
      }
    } else {
    ...
    <Stack gap={compact ? "md" : "xl"}>
      ...
      <Stack gap="sm">
          ...
          {[
            ...
            return (
              <Button
                ...
                size="sm"
                onClick={
                  readOnly ? undefined : () => onModeChange(m.id as QuizMode)
                }
                disabled={readOnly}
              >
                {m.label}
              ...
              <Flex
                ...
                onClick={
                  readOnly || isOffline
                    ...
                    : () =>
                        onModeChange(
                          preferredMode === "fuzzy" ? "text" : "fuzzy",
                        )
                }
              >
                {/* Toggle pill */}
        ...
        {preferredMode === "chips" && (
          <Stack
            gap="sm"
            className="animate-in fade-in slide-in-from-top-1 duration-300 mt-2"
          >
            ...
            <View className="px-2">
              <BaseRange
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  onDummyCountChange(parseInt(e.target.value))
                }
              />
      ...
      <Stack gap="sm">
        ...
        <Flex gap="sm" className="flex-wrap">
          {Object.entries(sortKeyLabels)
            .filter(([k]) => k !== SortKey.RANDOM)
            .map(([key, label]) => {
              ...
              const activeSortsOrdered = sorts.filter(
                (s) => s.key !== SortKey.RANDOM,
              );
              const sortRank =
                activeSortsOrdered.findIndex((s) => s.key === _key) + 1;
          ...
          <Stack gap="md">
            ...
            <Stack gap="xs">
              ...
              <BaseRange
                ...
                onChange={(e) =>
                  updateMatchConfig({ text_timer_s: parseInt(e.target.value) })
                }
                disabled={readOnly}
              />
            </Stack>
            ...
            <Stack gap="xs">
              <Flex justify="between" align="center">
                ...
                <Text variant="xs" weight="bold">
                  {matchConfig.chips_char_timer_s}s
                </Text>
              </Flex>
              <BaseRange
                min={1}
                ...
                onChange={(e) =>
                  updateMatchConfig({
                    chips_char_timer_s: parseInt(e.target.value),
                  })
                }
                disabled={readOnly}
              />
            ...
            <Stack gap="xs">
              ...
              <BaseRange
                ...
                onChange={(e) =>
                  updateMatchConfig({ base_score: parseInt(e.target.value) })
                }
                disabled={readOnly}
              />
            </Stack>
            ...
            <Stack gap="xs">
              <Flex justify="between" align="center">
                ...
                <Text variant="xs" weight="bold">
                  {matchConfig.speed_bonus_max}pts
                </Text>
              </Flex>
              <BaseRange
                min={0}
                ...
                onChange={(e) =>
                  updateMatchConfig({
                    speed_bonus_max: parseInt(e.target.value),
                  })
                }
                disabled={readOnly}
              />
            ...
            <Stack gap="xs">
              ...
              <BaseRange
                ...
                onChange={(e) =>
                  updateMatchConfig({ penalty: parseInt(e.target.value) })
                }
                disabled={readOnly}
              />
            </Stack>
