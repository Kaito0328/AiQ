...
export function AiGenerationModal({
  ...
  const { generate, reset, status, message, generatedQuestions } =
    useAiGeneration(collectionId);
  ...
  const handleGenerate = () => {
    if (!prompt.trim() && !pdfFile) {
      showToast({
        message: "トピック、指示、またはPDFを入力してください",
        variant: "warning",
      });
      return;
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="AIで問題作成" size="lg">
      <Stack gap="xl">
        <AiUsageDisplay />
        {status === "idle" ? (
          <>
            <Stack gap="sm">
              <Text weight="bold" variant="xs">
                トピック・指示
              </Text>
              <TextArea
                  ...
                  <View
                    className={cn(
                      ...
                      pdfFile
                        ...
                        : "border-surface-muted/50",
                    )}
                    onClick={() =>
                      document.getElementById("pdf-modal-upload")?.click()
                    }
                  >
                    <input
                      id="pdf-modal-upload"
                ...
                <>
                  ...
                  <Stack gap="md" className="w-full">
                    <Stack gap="xs" align="center">
                      ...
                      <Text variant="xs" color="secondary">
                        {generatedQuestions.length}問の問題を追加しました。
                      </Text>
                    </Stack>
