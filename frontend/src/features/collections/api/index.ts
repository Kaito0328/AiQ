...
export const getUserCollections = async (
  ...
  return await apiClient<Collection[]>(`/users/${userId}/collections`, {
    authenticated: true,
  });
};

export const getFolloweeCollections = async (
  limit = 20,
  offset = 0,
): Promise<Collection[]> => {
  return await apiClient<Collection[]>(
    `/timeline/followees?limit=${limit}&offset=${offset}`,
    { authenticated: true },
  );
};
...
export const getRecentCollections = async (
  ...
  return await apiClient<Collection[]>(
    `/timeline/recent?limit=${limit}&offset=${offset}`,
    { authenticated: true },
  );
};
...
export const getPopularCollectionTags = async (
  q?: string,
  limit: number = 20,
  ...
  return await apiClient<PopularTagItem[]>(
    `/collections/tags/popular?${query.toString()}`,
    { authenticated: true },
  );
};
...
export const batchCollections = async (
  data: BatchCollectionsRequest,
): Promise<void> => {
...
export const getUserCollectionSets = async (
  userId: string,
...
export const getCollectionSet = async (
  setId: string,
...
export const exportCsv = async (collectionId: string): Promise<Blob> => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL}/api/collections/${collectionId}/csv`,
    {
      headers: {
