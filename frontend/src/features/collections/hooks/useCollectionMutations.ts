...
import {
  ...
  updateCollection,
  deleteCollection,
  upsertCollectionSearchMetadata,
} from "../api";
...
export function useCollectionMutations() {
  ...
  const doCreateCollection = useCallback(
    async (data: CreateCollectionRequest): Promise<Collection> => {
      ...
      const hasSearchMetadataPayload =
        (Object.prototype.hasOwnProperty.call(data, "difficultyLevel") &&
  ...
  const doUpdateCollection = useCallback(
    async (id: string, data: UpdateCollectionRequest): Promise<Collection> => {
      const { tags, difficultyLevel, ...baseData } = data;
      const hasSearchMetadataPayload =
        (Object.prototype.hasOwnProperty.call(data, "difficultyLevel") &&
          difficultyLevel !== undefined) ||
      ...
      if (isOnline) {
        ...
        if (current) {
          ...
          await syncManager.addAction("UPDATE_COLLECTION", {
            id,
            data: baseData,
          });
  ...
  const doDeleteCollection = useCallback(
