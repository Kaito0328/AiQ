...
export function CollectionSetCard({
  ...
  const { collections, loading } = useCollectionSet(
    isExpanded ? set.id : undefined,
  );
      ...
      <Stack gap="md" className="h-full">
          ...
          <Stack gap="none" className="flex-1 pr-16">
            <Flex gap="xs" align="center">
              <Text
                weight="bold"
                variant="detail"
                className="line-clamp-1 group-hover:text-brand-primary transition-colors"
              >
                {set.name}
              </Text>
              {isPendingLocalSet && (
        ...
        <View className="flex-1">
          <Text
            variant="xs"
            color="secondary"
            className="line-clamp-2 leading-relaxed opacity-90"
          >
        ...
        <Flex
          justify="between"
          align="center"
          className="pt-4 border-t border-surface-muted/50"
        >
          <Text
            variant="xs"
            color="primary"
  ...
  return (
    <View className="relative group h-full">
      ...
      {isSelectionMode ? (
        ...
        <View className="block h-full cursor-default">{cardContent}</View>
      ) : (
        <OfflineLink
          href={`/collection-sets/${set.id}`}
