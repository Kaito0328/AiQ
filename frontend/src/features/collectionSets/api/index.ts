...
function isOfflineMode(): boolean {
  ...
  return (
    localStorage.getItem("aiq_manual_offline") === "true" || !navigator.onLine
  );
}
...
function createTemporarySet(req: CreateCollectionSetRequest): CollectionSet {
  const profileRaw =
    typeof window !== "undefined"
      ? localStorage.getItem("aiq_user_profile")
      : null;
...
export const addCollectionToSet = async (
  setId: string,
...
export const removeCollectionFromSet = async (
  setId: string,
  collectionId: string,
): Promise<void> => {
  await apiClient<void>(
    `/collection-sets/${setId}/collections/${collectionId}`,
    {
      method: "DELETE",
      authenticated: true,
    },
